...
class Pins {
    ...
    static loadPins() {
        ...
        if (Pins.isValidJSON(localStorage.getItem('gta3.pinned-items'))) {
            JSON.parse(localStorage.getItem('gta3.pinned-items')).forEach(pinnedItem => {
    ...
    static save() {
        localStorage.setItem('gta3.pinned-items', JSON.stringify(this.pinsList));
    }

    static importPins(text) {
        if (Pins.isValidJSON(text)) {
            console.log(text);
            localStorage.setItem('gta3.pinned-items', text);
            this.loadPins();
        } else {
    ...
    static exportPins() {
        const text = localStorage.getItem('gta3.pinned-items');
    ...
    static set onMap(state) {
        if (state) {
            ...
            if (!MapBase.isPreviewMode)
                localStorage.setItem('gta3.pins-enabled', 'true');
            this.context.removeClass('disabled');
        } else {
            this.layer.remove();
            if (!MapBase.isPreviewMode)
                localStorage.removeItem('gta3.pins-enabled');
    ...
    static get onMap() {
        return !!localStorage.getItem('gta3.pins-enabled');
    }
